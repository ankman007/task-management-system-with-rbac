...
def login(login_in: LoginRequest, db: Session = Depends(get_db)):
    user = UserService.authenticate_user(db=db, email=login_in.email, password=login_in.password)
...
def refresh_token_endpoint(refresh_token: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "refresh_token": new_tokens["refresh_token"], # Rotation pattern: send back a fresh refresh token too
        "token_type": "bearer"
    }